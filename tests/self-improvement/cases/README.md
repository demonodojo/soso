...
|---|---|---|---|
| `programacion` | 10 (`P01`–`P10`) | un programa Rust de un archivo contra vectores reservados | `verificadores/verificar_programa.py` |
| `protocolo` | 10 (`Q01`–`Q10`) | la respuesta HTTP/SSE grabada del servidor | `verificadores/verificar_protocolo.py` |
| `repo` | 5 (`R01`–`R05`) | un árbol del proyecto contra una prueba reservada | `verificadores/verificar_repo.py` |
...
```sh
python3 banco.py listar     # los 25 casos con su clase y partición
python3 banco.py validar    # estructura, esquema, hashes y huella
python3 banco.py sellar     # recalcula partición, hashes y huella tras un cambio
```
...
python3 verificadores/verificar_programa.py --caso P01 --candidato solucion.rs

# protocolo: el candidato es la respuesta grabada
python3 verificadores/verificar_protocolo.py --caso Q01 --respuesta respuesta.json

# repo: el candidato es el árbol modificado
python3 verificadores/verificar_repo.py --caso R01 --arbol /ruta/al/arbol
```
...
`verificar_repo.py` deja el árbol como lo encontró —borra la prueba que
...
```sh
python3 -m unittest discover -s tests/self-improvement -p test_cases.py
```
