...
impl Channel {
    ...
    fn handle_eof(&mut self, s: &mut TrafSend, is_client: bool) -> Result<()> {
        // PARCHE soso (ver vendor/README.md): NO se espeja el EOF del par.
        //
        // El original mandaba aquí un ChannelEof de vuelta —lo que anuncia "no
        // enviaré más datos"— sólo porque el par dejó de enviar. El EOF de SSH
        // es por dirección (RFC 4254 §5.3), así que eso es incorrecto: OpenSSH
        // lo recibe, hace "output drain -> closed" y descarta todo lo que el
        // servidor imprima después. Con soso eso convertía `printf 'cmd\n' |
        // ssh` en una sesión que ejecuta el comando y no devuelve nada.
        //
        // `sent_eof` queda sin usar: sólo se leía y escribía en este bloque.
        let _ = (&s, self.sent_eof);
