...
impl CommandRing {
    ...
    pub unsafe fn new() -> Self {
        let (ptr, ring_phys) = dma::alloc_aligned(RING_SEGMENT_SIZE, 64);
...
impl EventRing {
    ...
    pub unsafe fn new() -> Self {
        let (ring_ptr, ring_phys) = dma::alloc_aligned(RING_SEGMENT_SIZE, 64);
        let ring_va = ring_ptr as *const Trb;

        let (erst_ptr, erst_phys) = dma::alloc_aligned(
