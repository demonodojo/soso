...
impl InputContext {
    ...
    pub unsafe fn new(ctx_size: usize) -> Self {
        ...
        let (buffer, phys) = dma::alloc_aligned(total_size, 64);
...
impl Dcbaa {
    ...
    pub unsafe fn new(max_slots: u8, _ctx_size: usize, scratchpad_count: u32) -> Self {
        ...
        let (buffer_ptr, phys) = dma::alloc_aligned(array_size, 64);
        ...
        if scratchpad_count > 0 {
            ...
            let (sp_ptr, sp_phys) = dma::alloc_aligned(sp_array_size, 64);
            let sp_array = sp_ptr as *mut u64;

            for i in 0..scratchpad_count as usize {
                let (page, page_phys) = dma::alloc_aligned(4096, 4096);
                ptr::write_volatile(sp_array.add(i), page_phys);
                let _ = page;
            }

            ptr::write_volatile(buffer, sp_phys);
        }
    ...
    pub unsafe fn alloc_device_context(&mut self, slot_id: u8, ctx_size: usize) -> u64 {
        ...
        let (va, phys) = dma::alloc_aligned(total_size, 64);
