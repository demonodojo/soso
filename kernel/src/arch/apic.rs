...
fn mmio_read(off: u32) -> u32 {
    unsafe {
        core::ptr::read_volatile(crate::mm::phys_to_virt(XAPIC_MMIO + off as u64).as_ptr())
    }
}
...
pub fn arrancar_ap(apic_id: u32, tramp_page: u8) {
    let espera = |ms: u64| {
        let fin = crate::arch::pit::uptime_ms() + ms;
        while crate::arch::pit::uptime_ms() < fin {
            core::hint::spin_loop();
        }
    };
    // INIT (assert de nivel) y dos SIPI con el vector de la página
    icr(apic_id, (0b101 << 8) | (1 << 14));
    espera(10);
...
pub fn timer_periodico(hz: u32) {
    ...
    let t0 = crate::arch::pit::uptime_ms();
    while crate::arch::pit::uptime_ms() < t0 + 50 {
        core::hint::spin_loop();
    }
