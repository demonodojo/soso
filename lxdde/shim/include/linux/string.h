...
{
	for (; *a && __lx_lc(*a) == __lx_lc(*b); a++, b++)
		;
	return __lx_lc((unsigned char)*a) - __lx_lc((unsigned char)*b);
}
static inline int strncasecmp(const char *a, const char *b, size_t n)
{
	for (; n && *a && __lx_lc(*a) == __lx_lc(*b); a++, b++, n--)
		;
	return n ? __lx_lc((unsigned char)*a) - __lx_lc((unsigned char)*b) : 0;
}
